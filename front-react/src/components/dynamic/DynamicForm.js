...
export default function DynamicForm({ metadata, initialData = {}, onSubmit, onClose, keycloak, realm, isNestedForm = false }) {
  ...
  const handleAutocompleteChange = (key, value) => {
    setForm((prev) => ({ ...prev, [key]: value ? { id: value.id } : null }));
  };
  ...
  const fetchOptions = async (relatedType, key, search = "") => {
    try {
      const res = await fetch(`${apiUrl}/${relatedType}?page=0&size=10&search=${encodeURIComponent(search)}`, {
        headers: {
                  ...
                  <div style={{ flexGrow: 1 }}>
                    <Autocomplete
                      ...
                      getOptionLabel={(option) => {
                        if (!option) return "";
                        const showField = metadata.properties[key]?.showField || "id";
                        return option[showField] ?? String(option.id ?? "");
                      }}
                      value={autocompleteOptions[key]?.find(o => o.id === form[key]?.id) || null}
                      ...
                      <Dialog open={!!openOneToOneModals[key]} onClose={() => setOpenOneToOneModals(prev => ({ ...prev, [key]: false }))} maxWidth="sm" fullWidth>
                        ...
                        <DialogContent style={{ paddingTop: 16 }}>
                          <DynamicForm
                            ...
                            onSubmit={async (val) => {
                              try {
                                const res = await fetch(`${apiUrl}/${meta.relatedType}`, {
                                  method: "POST",
                                  headers: {
                                    Authorization: "Bearer " + keycloak.token,
                                    Tenant: realm,
