...
import br.com.mindhaven.domain.repository.UserRepository;
...
public class UserUseCaseImpl implements UserUseCase {
    ...
    private final MinioUseCaseImpl minioUseCase;

    public UserUseCaseImpl(UserRepository userRepository, KeycloakServiceImpl keycloakService, MinioUseCaseImpl minioUseCase) {
        this.userRepository = userRepository;
        this.keycloakService = keycloakService;
        this.minioUseCase = minioUseCase;
    }
    ...
    public UserEntity findByKeycloakId(String tenant, String keycloakId) {
        UserEntity user = keycloakService.findByKeycloakId(tenant, keycloakId);
        byte[] image = minioUseCase.getObjectBytes(tenant + "/user/" + user.getId() + "/user_profile.png");
        if (image != null) {
            user.setProfileImage(java.util.Base64.getEncoder().encodeToString(image));
        }
        return user;
    }
}
