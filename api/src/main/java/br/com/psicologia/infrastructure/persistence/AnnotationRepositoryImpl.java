...
import br.com.psicologia.domain.repository.AnnotationRepository;
...
public class AnnotationRepositoryImpl implements AnnotationRepository {
    ...
    @Override
    public List<AnnotationEntity> filteredFindPagedComPermissao(String tenant, br.com.psicologia.domain.entity.UserEntity loggedUser, Object filter, int page, int size) {
        if (loggedUser.getUserType().equals(br.com.psicologia.domain.entity.enums.UserType.ADMINISTRADOR)) {
            return filteredFindPaged(tenant, filter, page, size);
        }
        List<AnnotationEntity> all = filteredFindPaged(tenant, filter, page, size);
        if (loggedUser.getUserType().equals(br.com.psicologia.domain.entity.enums.UserType.PSICOLOGO)) {
            return all.stream()
        ...
        if (loggedUser.getUserType().equals(br.com.psicologia.domain.entity.enums.UserType.PACIENTE)) {
            return all.stream()
                .filter(a -> a.getSession() != null &&
                    a.getSession().getSessionPackage() != null &&
    ...
    @Override
    public long countFilteredComPermissao(String tenant, br.com.psicologia.domain.entity.UserEntity loggedUser, Object filter) {
